package th.co.geniustree.internship.faris.microioc.container;
...
public class MyContainer {
    ...
    private static List<Class> scanClassPath() {
        ...
        List<Class> classes = new FastClasspathScanner().scan().getNamesOfAllClasses()
                .stream()
                .filter(e -> e.startsWith("th.co.geniustree.internship"))
    ...
    private static void run(Object instance) {
        try {
            Method microioc = instance.getClass().getDeclaredMethod("microioc", null);
            System.out.println("running method microioc()");
            microioc.invoke(instance, null); //against
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
